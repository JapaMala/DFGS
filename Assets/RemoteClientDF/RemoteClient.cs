...
namespace DFHack
{
    ...
    public struct BlockCoord
    {
        ...
        {
            if (coord.x % blockSize != 0 || coord.y % blockSize != 0)
            {
                throw new InvalidOperationException("Can't make a block coord from a non-block-corner");
            }
            return new BlockCoord(coord.x / blockSize, coord.y / blockSize, coord.z);
        }

        public DFCoord ToDFCoord()
        {
            return new DFCoord(x * blockSize, y * blockSize, z);
        }

        public override string ToString()
        {
            return string.Format("BlockCoord({0}[{1}],{2}[{3}],{4})", x, x * blockSize, y, y * blockSize, z);
        }
